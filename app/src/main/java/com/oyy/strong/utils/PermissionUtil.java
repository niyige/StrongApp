...
public class PermissionUtil {
    ...
    public static void requestPermissionFragment(Fragment fragment, final Context mContext, final int requestCode, PermissionGrant permissionGrant) {
        if (mContext == null) {
            return;
        }
...
//            Toast.makeText(mContext, "please open this permission", Toast.LENGTH_SHORT)
//                    .show();
        ...
        if (checkSelfPermission != PackageManager.PERMISSION_GRANTED) {
            ...
            if (fragment.shouldShowRequestPermissionRationale(requestPermission)) {
                ...
                shouldShowRationale(mContext,fragment, requestCode, requestPermission);
            } else {
...
//            Toast.makeText(mContext, "opened:" + requestPermissions[requestCode], Toast.LENGTH_SHORT).show();
...
//                    .show();
            ...
            openSettingActivity(activity, "请求获取权限");
        }
    ...
    public static void requestMultiPermissions(final Activity activity, String[] requestPermissions, PermissionGrant grant) {
        ...
        if (permissionsList.size() > 0) {
            ...
            showMessageOKCancel(activity, "请求获取权限",
     ...
     * 一次申请多个权限 Fragment
     */
    public static void requestMultiPermissionsFragment(final Fragment fragment, Context mContext, String[] requestPermissions, PermissionGrant grant) {

        final List<String> permissionsList = getNoGrantedPermission((Activity) mContext,requestPermissions, false);
        final List<String> shouldRationalePermissionsList = getNoGrantedPermission((Activity) mContext,requestPermissions, true);
        ...
        if (permissionsList.size() > 0) {
            ...
            showMessageOKCancel(mContext, "请求获取权限",
                    new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
    ...
    private static void shouldShowRationale(Context context, final Fragment activity, final int requestCode, final String requestPermission) {
        //TODO
        String[] permissionsHint = context.getResources().getStringArray(R.array.permissions);
        showMessageOKCancel(context, "Rationale: " + permissionsHint[requestCode], new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
    ...
    private static void showMessageOKCancel(final Context context, String message, DialogInterface.OnClickListener okListener) {
        new AlertDialog.Builder(context)
                .setMessage(message)
                .setPositiveButton(context.getString(R.string.enter), okListener)
...
//                        .show();
     ...
     * @return
     */
    public boolean selfPermissionGranted(String permission, Context mContext) {
